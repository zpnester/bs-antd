...
module MonthPicker = {
  ...
  external make:
    ...
    // ***** BEGIN MONTH PICKER FROM WEBSITE *****
      ~autoFocus: bool=?,
      ~monthCellContentRender: (Moment.t, Locale.t('todo)) =>
                               element
                                 =?,
...
module RangePicker = {
  ...
  external make:
    ...
    // ~className: string=?,
      ...
      ~mode: (Mode.t, Mode.t)=?,
      ~separator: element=?,
      ~onPanelChange: (array(Js.null(Moment.t)), array(Js.null(Mode.t))) => unit=?,
      // ***** END RANGE *****
