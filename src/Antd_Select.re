...
external make:
  (
    ~ref: React.Ref.t(Js.nullable(Ref.t))=?,
    ~allowClear: bool=?,
...
module Default = {
    include Make({
        type selectValue = option({ . "key": string, "label": element });
        type optionElement = option(element);
        type selectValueNotNull = { . "key": string, "label": element }
        type selectValueSingle = { . "key": string, "label": element }
    });
    let makeProps = makeProps(~mode="default", ~labelInValue=true, 
    ~maxTagPlaceholder=?None, ~maxTagCount=?None,
    ~onDeselect=?None, ~tokenSeparators=?None, ~autoClearSearchValue=?None);
...
module Multiple = {
    ...
    include Make({
        type selectValue = array({ . "key": string, "label": element });
        type selectValueSingle = { . "key": string, "label": element };
        type optionElement = array(element);
        type selectValueNotNull = array({ . "key": string, "label": element })

    });

    let makeProps = makeProps(~mode="multiple", ~labelInValue=true,
    ~showSearch=?None);
};
...
module Tags = {
    ...
    let makeProps = makeProps(~mode="tags", ~labelInValue=true,
    ~showSearch=?None);
};
...
module OptGroup = {
  [@react.component] [@bs.module "antd/lib/select"]
  external make: (~label: element=?, ~key: string=?, ~children: element=?, unit) => element =
    "OptGroup";
};
