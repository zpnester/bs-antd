...
let expectPartial = (p: RangePicker.Partial.t) => {
    expectToEqual(p ==  RangePicker.Partial.start || p ==  RangePicker.Partial.end_, true);
};
...
let make = () => {
  ...
  <>
    ...
    dateRender={(~current, ~today) => {
        ...
        expectMoment(today);
        string(current |> MomentRe.Moment.format("MMMM DD"))
    }}
    ...
    <DatePicker.RangePicker
        ranges=Js.Dict.fromList([
            ("A", () => (
                Js.Null.return(momentNow() |> Moment.subtract(~duration=duration(5.0, `days))), 
                Js.Null.return(momentNow() |> Moment.add(~duration=duration(1.0, `days))
            )
            )),
            ("B", () => (
                Js.Null.return(momentNow() |> Moment.subtract(~duration=duration(5.0, `days))), 
