...
let make = () => {
  ...
  <>
    ...
    <button onClick={_ => car->getCarExn->next}> {string("Next")} </button>
    <button onClick={_ => car->getCarExn->prev}> {string("Prev")} </button>
    <button onClick={_ => car->getCarExn->goTo(2, false)}>
      {string("Go to 2")}
    </button>
