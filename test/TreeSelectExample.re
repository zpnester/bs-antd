...
let make = () => {
    ...
    let (value, setValue) = useState(() => [|{"value": "1-0", "label": null}|]);

    let (value2, setValue2) = useState(() => Some({"value": "1-0", "label": null}));

    let t = <b style={ReactDOMRe.Style.make(~color="#08c", ())}>{string("sss")}</b>;

    let treeData = [|
      Node.make(~value="a", ~title=string("A"), 
        ~children=[|
          Node.make(~value="aa", ~title=string("AA"), ())
        |],
      ()),
    ...
    <>
        ...
        multiple=Multiple.true_
            ...
            value
            dropdownStyle={ReactDOMRe.Style.make(~maxHeight="400px", ~overflow="auto", ())}
      ...
      >
        <TreeNode value="parent 1" title={<h2>{string("parent 1")}</h2>} key="0-1">
          ...
          <TreeNode value="parent 1-1" 
            label=string("Parent 1-1 label")  title=string("parent 1-1") key="random2">
